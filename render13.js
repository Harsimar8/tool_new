/* ==========================================================================
   RENDER ENGINE — AEROSPACE DEFENSE MISSION EDITOR C4ISR SUITE
   ========================================================================== */

document.addEventListener("DOMContentLoaded", async () => {
    try {
        // 1. Ensure MilSymbol CDN is dynamically loaded
        if (!window.ms) {
...
function renderSections(sections) {
    ...
    sections.forEach(section => {
        ...
        // 2. ALTERNATING ZIG-ZAG BLADES (ONE LEFT, ONE RIGHT LINE BY LINE)
        else {
            const buttonItems = section.items.filter(item => item.type === "button" || item.type === "mode");
            const sliderItems = section.items.filter(item => item.type === "number");
            const toggleItems = section.items.filter(item => item.type === "toggle");

            if (buttonItems.length > 0) {
                html += `<div class="staggered-flank-container">`;
                ...
                buttonItems.forEach((item, idx) => {
                    ...
                    const tagText = item.value ? item.value : (isAction ? "EXEC" : "ACT");

                    html += `
                        <div class="flank-button-wrapper">
                            <button class="flank-tool-button${dangerClass}" onclick="${clickAction}">
                                <div class="flank-btn-left">
                                    <span class="flank-btn-icon">${item.icon || "◈"}</span>
                                    <span>${item.label}</span>
                                </div>
                                <span class="flank-btn-tag">${tagText}</span>
                            </button>
                        </div>
                    `;
                });
            ...
            toggleItems.forEach(item => {
                html += `
                    <button id="${item.id}Button" class="tactical-toggle-btn" 
...
function createMilSymbolSVG(sidc, name = "", size = 36) {
    ...
    if (lower.includes("radar")) {
        return `<svg width="${size}" height="${size}" viewBox="0 0 36 36" fill="none">
            <rect x="4" y="8" width="28" height="20" rx="3" stroke="#38bdf8" stroke-width="2" fill="rgba(56,189,248,0.12)"/>
            <path d="M11 20a7 7 0 0 1 14 0" stroke="#38bdf8" stroke-width="1.8" stroke-linecap="round"/>
            <path d="M14 20a4 4 0 0 1 8 0" stroke="#f59e0b" stroke-width="1.8" stroke-linecap="round"/>
            <circle cx="18" cy="20" r="1.5" fill="#f59e0b"/>
        </svg>`;
    } else if (lower.includes("tank")) {
        return `<svg width="${size}" height="${size}" viewBox="0 0 36 36" fill="none">
            <rect x="4" y="8" width="28" height="20" rx="3" stroke="#38bdf8" stroke-width="2" fill="rgba(56,189,248,0.12)"/>
            <ellipse cx="18" cy="18" rx="8" ry="4.5" stroke="#f59e0b" stroke-width="1.8" fill="none"/>
            <circle cx="18" cy="18" r="2" fill="#38bdf8"/>
        </svg>`;
    } else if (lower.includes("sam") || lower.includes("s-400")) {
        return `<svg width="${size}" height="${size}" viewBox="0 0 36 36" fill="none">
            <polygon points="18 4 32 30 4 30" stroke="#38bdf8" stroke-width="2" fill="rgba(56,189,248,0.12)"/>
            <path d="M18 10v14M14 20l4-6 4 6" stroke="#f59e0b" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/>
        </svg>`;
    } else if (lower.includes("missile")) {
        return `<svg width="${size}" height="${size}" viewBox="0 0 36 36" fill="none">
            <rect x="4" y="8" width="28" height="20" rx="3" stroke="#38bdf8" stroke-width="2" fill="rgba(56,189,248,0.12)"/>
            <path d="M18 10l5 12h-10l5-12z" stroke="#f59e0b" stroke-width="1.8" fill="rgba(245,158,11,0.2)"/>
        </svg>`;
    } else if (lower.includes("bomber") || lower.includes("aircraft")) {
        return `<svg width="${size}" height="${size}" viewBox="0 0 36 36" fill="none">
            <path d="M6 24c4-12 20-12 24 0" stroke="#38bdf8" stroke-width="2" fill="rgba(56,189,248,0.12)"/>
            <path d="M18 10v14M10 18h16" stroke="#f59e0b" stroke-width="1.8" stroke-linecap="round"/>
        </svg>`;
    }

    return `<svg width="${size}" height="${size}" viewBox="0 0 36 36" fill="none">
        <rect x="4" y="8" width="28" height="20" rx="3" stroke="#38bdf8" stroke-width="2" fill="rgba(56,189,248,0.12)"/>
        <circle cx="18" cy="18" r="5" stroke="#f59e0b" stroke-width="1.8" fill="none"/>
    </svg>`;
}
