/* ==========================================================================
   INTERACTION CONTROLLER — AEROSPACE DEFENSE MISSION EDITOR C4ISR SUITE
   ========================================================================== */
...
function switchSection(sectionId, btn) {
    ...
    if (target) {
        ...
        target.style.animation = "c2SlideIn 0.22s cubic-bezier(0.4, 0, 0.2, 1)";
    }

    setStatus(`Node Armed: ${sectionId.toUpperCase()}`);
}
...
function sliderChange(id, value, unit) {
    ...
    if (slider) {
        ...
        slider.style.background = `linear-gradient(90deg, #38bdf8 0%, #38bdf8 ${pct}%, rgba(255, 255, 255, 0.15) ${pct}%)`;
    }
...
function toggleMaskSetting(button, id, onText, offText) {
    ...
    setStatus(`${id}: ${nextState ? "ONLINE" : "STANDBY"}`);
}
...
function initDockResizer() {
    ...
    const stopResize = (e) => {
        if (isResizing) {
            ...
            setStatus(`HUD Resized: ${dock.offsetWidth}×${dock.offsetHeight}px`);
        }
    };
...
function selectOption(button, id, name) {
    ...
    if (parent) {
        parent.querySelectorAll(".flank-tool-button.selected").forEach(btn => btn.classList.remove("selected"));
    }
...
function performAction(id, name) {
    document.querySelectorAll(".flank-tool-button.selected").forEach(btn => btn.classList.remove("selected"));
    showNotification(name);
...
function setStatus(message) {
    ...
    statusTimer = setTimeout(() => { status.textContent = "Mission editor ready"; }, 2500);
}
